...
async def get_provider_listings(address: str, limit: int = Query(50, le=200)):
    ...
    return [l for l in all_listings if l.provider_address == address]
...
async def get_provider_stats(address: str):
    ...
    provider_listings = [l for l in all_listings if l.provider_address == address]

    return {
        "address": address,
        "active_listings": len(provider_listings),
        "total_value_lovelace": sum(l.price_lovelace for l in provider_listings),
        "avg_reputation": (
            sum(l.reputation_score for l in provider_listings) / len(provider_listings)
